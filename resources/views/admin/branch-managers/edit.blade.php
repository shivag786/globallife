<x-layouts.app title="Edit Branch Manager" heading="Edit Branch Manager">
    ...
    {{-- Its own form, a sibling of the one above: nesting forms is invalid HTML,
         and a password reset should be a deliberate action rather than something
         that rides along with a profile save. --}}
    <div class="bootstrap-scope max-w-3xl mt-4">
        <div class="card shadow-sm border-0">
            <div class="card-header bg-white">
                <h2 class="h6 mb-0 fw-semibold">Change Password</h2>
                <p class="small text-muted mb-0">
                    Sets a new password for <strong>{{ $manager->name }}</strong>. They are signed out
                    everywhere and must use the new one &mdash; you are not shown the current password,
                    because it is only ever stored hashed.
                </p>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('admin.branch-managers.password.update', $manager) }}"
                      data-confirm="Set a new password for {{ $manager->name }}? This signs them out of every device immediately."
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            <label for="bm-password" class="form-label small fw-medium text-slate-700">New Password</label>
                            <input id="bm-password" type="password" name="password" required minlength="8"
                        ...
                        <div class="col-md-6">
                            <label for="bm-password-confirm" class="form-label small fw-medium text-slate-700">Confirm New Password</label>
                            <input id="bm-password-confirm" type="password" name="password_confirmation" required
